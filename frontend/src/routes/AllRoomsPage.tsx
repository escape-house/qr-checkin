...
function AllRoomsPage() {
    ...
    return (
        <div className="max-w-lg mx-auto px-4 py-6">
            ...
            {isPending && <KeyScreen />}
            {isError && <p className="text-error">{t("roomsErrorLoading")}</p>}
            {rooms?.length === 0 && <p className="text-text-secondary">{t("roomsNoRooms")}</p>}

            {rooms?.map(room => (
                <div
                    key={room}
                    className="border border-border rounded-lg overflow-hidden mb-3 cursor-pointer bg-surface hover:border-primary transition-colors"
                    onClick={() => navigate(`/rooms/${encodeURIComponent(room.replaceAll(" ", "_"))}`)}
                >
                    <img
