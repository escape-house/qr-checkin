...
export function SlotCard({slot, status}: Props) {
    ...
    return (
        <div className={`rounded-[--radius-md] flex flex-row overflow-hidden transition-colors ${cardStyles[status]}`}>
            {/* Left accent bar */}
            ...
            <div className="px-3 py-2 flex flex-col gap-1">
                ...
                {/* Progress + count */}
                    <div className={`flex flex-col gap-1 mt-0.5 ${booked?"":"opacity-0"}`}>
                        <CheckInProgressBar checkedIn={checkedIn} total={total} status={status} />
                        <div className="flex items-center gap-1.5">
                            <span className={`text-xs font-medium px-1.5 py-0.5 rounded ${badgeStyles[status]}`}>
                                {checkedIn}/{total}
                            </span>
